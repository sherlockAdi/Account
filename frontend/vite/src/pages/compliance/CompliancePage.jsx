...
export default function CompliancePage() {
  ...
  const filteredRules = useMemo(() => rules.filter((rule) =>
    ...
    (!filters.onDate || (rule.effectiveFrom.slice(0, 10) <= filters.onDate && (!rule.effectiveTo || rule.effectiveTo.slice(0, 10) >= filters.onDate)))
  ), [rules, filters]);
    ...
    <Grid container spacing={2.75}>
      ...
      <Dialog open={ruleOpen} onClose={() => setRuleOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogContent><Grid container spacing={2} sx={{ mt: 0.5 }}>
          ...
          <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth type="date" label="Effective From" value={ruleForm.effectiveFrom} onChange={(e) => setRuleForm({ ...ruleForm, effectiveFrom: e.target.value })} InputLabelProps={{ shrink: true }} /></Grid>
          <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth type="date" label="Effective To" value={ruleForm.effectiveTo} onChange={(e) => setRuleForm({ ...ruleForm, effectiveTo: e.target.value })} InputLabelProps={{ shrink: true }} /></Grid>
