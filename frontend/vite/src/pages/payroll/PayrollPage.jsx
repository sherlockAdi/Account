    ...
    <Grid container spacing={2.75}>
      ...
      <Dialog open={employeeOpen} onClose={() => setEmployeeOpen(false)} fullWidth maxWidth="md">
        <Box component="form" onSubmit={(event) => {
          ...
          <DialogContent><Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth type="date" label="Date of Joining" value={employeeForm.dateOfJoining} onChange={(e) => setEmployeeForm({ ...employeeForm, dateOfJoining: e.target.value })} InputLabelProps={{ shrink: true }} required /></Grid>
      ...
      <Dialog open={salaryOpen} onClose={() => setSalaryOpen(false)} fullWidth maxWidth="md">
        <Box component="form" onSubmit={(event) => {
          ...
          <DialogContent><Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid size={6}><TextField fullWidth type="date" label="Effective From" value={salaryForm.effectiveFrom} onChange={(e) => setSalaryForm({ ...salaryForm, effectiveFrom: e.target.value })} InputLabelProps={{ shrink: true }} /></Grid>
            <Grid size={6}><TextField fullWidth type="date" label="Effective To" value={salaryForm.effectiveTo} onChange={(e) => setSalaryForm({ ...salaryForm, effectiveTo: e.target.value })} InputLabelProps={{ shrink: true }} /></Grid>
...
function ProcessingPanel({ period, setPeriod, preview, runForm, setRunForm, branches, onProcess }) {
  return <Stack spacing={2.5} sx={{ p: 2.5 }}>
    ...
    <Card variant="outlined"><CardContent><Typography variant="h5" sx={{ mb: 2 }}>Post Payroll</Typography><Grid container spacing={2}><Grid size={{ xs: 12, md: 3 }}><TextField fullWidth label="Run No" value={runForm.runNo} onChange={(e) => setRunForm({ ...runForm, runNo: e.target.value })} /></Grid><Grid size={{ xs: 12, md: 3 }}><TextField fullWidth type="date" label="Payment Date" value={runForm.paymentDate} onChange={(e) => setRunForm({ ...runForm, paymentDate: e.target.value })} InputLabelProps={{ shrink: true }} /></Grid><Grid size={{ xs: 12, md: 3 }}><TextField select fullWidth label="Branch Scope" value={runForm.branchId} onChange={(e) => setRunForm({ ...runForm, branchId: e.target.value })}><MenuItem value="">All Branches</MenuItem>{branches.map((branch) => <MenuItem key={branch.id} value={branch.id}>{branch.name}</MenuItem>)}</TextField></Grid><Grid size={{ xs: 12, md: 3 }}><Button fullWidth variant="contained" size="large" disabled={!preview.lines.length} onClick={onProcess}>Process & Post</Button></Grid></Grid></CardContent></Card>
  </Stack>;
}
